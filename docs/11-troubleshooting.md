...
**Error (LOCAL mode EKS blueprint):**
```
Error: kind cluster 'gentepede-local' not found.
Run: kind create cluster --name gentepede-local
Then re-run this tool.
```

**Root cause:** Either `kind` is not installed, or the `gentepede-local` cluster was not created.
...
## LocalStack not running (Connection refused localhost:4566)

**Error:** Terraform outputs `connection refused` or `no such host` when running in LOCAL mode.

**Root cause:** LocalStack Docker container is not running.
...
If the container exists but is stopped: `docker start localstack`

---

## kind cluster `gentepede-local` not found (EKS LOCAL)
...
`kind-config.yaml` is generated by `generate_infrastructure_package` in the workspace root. If the workspace was not yet generated, generate it first, then create the cluster.

---

## No AWS credentials (PRODUCTION mode)
...
## Remote state S3 bucket / DynamoDB table not found (PRODUCTION first apply)

**Error:** Terraform fails with `BucketNotFound` or `ResourceNotFoundException`

**Root cause:** The S3 state bucket and DynamoDB lock table must exist before the first `terraform apply` in PRODUCTION mode. They cannot be created by Terraform themselves (chicken-and-egg problem).

**Fix:** See `docs/12-end-to-end-walkthrough.md` Phase 7 for the exact setup commands. Create them once per project via the AWS CLI.
